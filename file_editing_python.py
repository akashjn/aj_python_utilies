...
from rmsd import test_rmsd

class GaussianOut: # Modified Garvit's script
    def __init__(self,outfile):
        f=open(outfile,'r')
        self.lines=f.readlines()  # whole file stored in the list; # list[i] = i_th line of file
        self.name = os.path.splitext(os.path.basename(f.name))[0] # file name without extension
        self.file_exe = os.path.splitext(os.path.basename(f.name))[1] # file extension
...
#        """ get geometry as an ASE object """
        ...
        result=[]
        for i in range(len(self.lines)): # iter through each line of the input file
            for item in str: # find the row number of "Standard orientation:"
                if item in self.lines[i]:
                    start = i
                    for m in range(start + 5, len(self.lines)):
                        if "---" in self.lines[m]:
                            end = m
                            break  # will only break out the "for m in range" loop; for i loop will continue  
                    Natoms = end - (start + 5)
                    sym = []
                    ...
                    # geometry.write(self.name+str(count)+".xyz")
    ...
    def get_rmsd(self):
        # initial_xyz=self.get_xyz_paths()[0]
        rmsd_wrt_initial_xyz=[]
        for idx,xyz in enumerate(self.get_xyz_paths()):
            if idx==0:
                initial_xyz=xyz
            
            rmsd_wrt_initial_xyz.append(test_rmsd.main(initial_xyz,xyz))
